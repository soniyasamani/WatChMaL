...
def plot_classifier_response(softmaxes, labels, particle_names, label_dict, 
    ...
    for independent_particle_label, ax in enumerate(axes[:softmaxes.shape[1]]):
        #print(label_dict)
        #print(independent_particle_label)
        ...
        for dependent_particle_label in dependent_particle_labels:
            #print(dependent_particle_label)
            ...
            ax.hist(softmaxes_list[dependent_particle_label][:,independent_particle_label],
                    ...
                    #linestyle=linestyles[dependent_particle_label],
                    linewidth=2)            
...
def plot_roc(fpr, tpr, thr,true_label_name, false_label_name, fig_list=None, xlims=None, ylims=None, axes=None, linestyle=None, linecolor=None, plot_label=None, show=False):
    ...
    if 0 in fig_list: 
        ...
        ax0.plot(fpr, tpr,
                    label=plot_label if plot_label  is not None else r'ResNet18, AUC={:.3f}'.format(roc_AUC),
    ...
    if 1 in fig_list: 
        ...
        ax1.plot(tpr, rejection, 
                    label=plot_label + ', AUC={:.3f}'.format(roc_AUC)  if plot_label is not None else r'ResNet18, AUC={:.3f}'.format(roc_AUC),
                    linestyle=linestyle  if linestyle is not None else None,
                    color=linecolor if linecolor is not None else None,
                    linewidth=2)     
    ...
    if 2 in fig_list: 
        ...
        ax2.plot(tpr, tpr/np.sqrt(fpr), 
                    label= plot_label + ', AUC={:.3f}'.format(roc_AUC) if plot_label is not None else r'{} VS {} CNN, AUC={:.3f}'.format(true_label_name, false_label_name, roc_AUC),
                    linestyle=linestyle  if linestyle is not None else None,
                    color=linecolor if linecolor is not None else None,
                    linewidth=2)
        ax2.plot(tpr2, tpr2/np.sqrt(fpr2), 
                    label= plot_label + ', AUC={:.3f}'.format(roc_AUC2) if plot_label is not None else r'{} VS {} BDT, AUC={:.3f}'.format(true_label_name, false_label_name, roc_AUC2),
                    linestyle=linestyle  if linestyle is not None else None,
                    color=linecolor if linecolor is not None else None,
                    linewidth=2)
...
def plot_rocs(softmax_out_val, labels_val, labels_dict, plot_list=None, vs_list = None, show=True,overlay=False):
    """
    ...
    for true_label_name in plot_list:
        ...
        for false_label_name in vs_list:
            ...
            if not (false_label_name == true_label_name):
                ...
                roc_figs=plot_roc(fpr,tpr,thr,true_label_name,false_label_name, fig_list=[0,1])
                for idx,fig in enumerate(roc_figs):
                    fig_name = f"/opt/ppd/hyperk/Users/samanis/WatChMaL/analysis/{true_label_name}_vs_{false_label_name}_{idx}.png"
...
path='/opt/ppd/hyperk/Users/samanis/WatChMaL/outputs/resnet18/test'
softmaxes=np.load("%s/softmax.npy"%(path))
labels=np.load("%s/labels.npy"%(path))



#disp_learn_hist_smoothed('outputs',losslim=2, window_train=1,window_val=1)

#plot_classifier_response(softmaxes,labels,['Signal','Background'],{'Signal':1,'Background':0},legend_locs = ['upper center','upper center'], bins = 50)

plot_rocs(softmaxes,labels,{'DSNB':1,'NCQE':0},plot_list=['DSNB'],vs_list=['NCQE'])
